...
import { Loader2, CheckCircle, AlertCircle, TruckIcon, FileText, RefreshCw, Search } from "lucide-react";
              ...
              {isLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  {ordersData && ordersData.total > limit && (
                    <PaginationControls total={ordersData.total} />
                  )}
                </div>
              )}
            </TabsContent>
            ...
            {isLoadingAll ? (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {searchTerm ? `No orders found matching "${searchTerm}"` : "No orders in the system"}
                  </CardDescription>
                </CardHeader>
              </Card>
            ) : (
              <div className="space-y-6">
                {allOrdersData?.orders?.map((order: Order) => (
                  <OrderCard key={order.id} order={order} />
                ))}
                
                {/* Pagination controls */}
                {allOrdersData && allOrdersData.total > limit && (
                  <PaginationControls total={allOrdersData.total} />
                )}
              </div>
            )}
          </div>
